...
<div id="wrapper">
  ...
  <div id="content">
    ...
    function update(no){
    var quantity = document.getElementById('qty_'+no).value;
    var price = document.getElementById('price_'+no).value;
    price = parseInt(price);
    quantity = parseInt(quantity);
    var amount= quantity * price ;
    document.getElementById('totalP_'+no).value = amount.toFixed(3);
}
    </script>
	<p>
    <table>
...
include('Conn.php');
		...
		$sql="SELECT products.Pro_ID, products.Pro_Name, products.Descp, products.Price, products.Dis_Price, products.MainPic, tblcart.TrnID, tblprocat.Cat_Name FROM(products INNER JOIN tblprocat ON products.Cat_ID = tblprocat.Cat_ID)INNER JOIN tblcart ON ( products.Pro_ID = tblcart.Pro_ID) ";
 ...
 if ($count!=0){
	 $i=0;
      ...
      <div class="product_box margin_r35">
        <div >
         <a href="#"><img src="<?php echo $row['Cat_Name'].'/'.$row['MainPic'];?>" alt="" width="190" height="100"/>
         </a> </div>
        <p class="price">Category: <?php echo $row['Cat_Name'];?></p>
         <p class="price">Selling Price: OR<?php echo $row['Price'];?></p>
		<p class="price">Price discount:OR<input type="text" class="priceTxtF" id="price_<?php echo $i;?>" value="<?php echo $row['Dis_Price'];?>" readonly="readonly"/></p>
         <p class="price">Quntity: <input type="number" class="priceTxtF" id="qty_<?php echo $i;?>" max="100" min="1" step="1" onChange="no = <?php echo $i;?>; update(no)" value="1">
       
         </p>
        
         
          <p class="price">Total: <input type="text" class="priceTxtF" id="totalP_<?php echo $i;?>" value="<?php echo $row['Dis_Price'];?>" />
		  
           <br />        
         
          <a href="remove.php?ID=<?php echo $row['TrnID'];?>">Remove</a>
    </div>
		<?php }
		?>
        ...
        <td>
         <a href="addCart.php?ID=<?php echo $row['Pro_ID'];?>"><a href="PayWindow.html"><input type="submit" value="Continu to Buy" /></a></a>
         </td></tr></table></p>
         </div>
	<br>
	 <br>
 ...
 <script>
function myFunction() {
alert("offers page is a spicial page for user who rigist in our website.It is include the product with discount");
}
</script>
 </font>
    </div>
