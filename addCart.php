...
include('conn.php');
        if(isset($_SESSION['UserName'])){
			...
			$sql="SELECT * FROM $tbl_name WHERE UserID=$UserID and Pro_ID=$ProID ";
		    $result=mysql_query($sql,$con);
			$count=mysql_num_rows($result);
			if ($count==0){
				$sqlInsert="Insert into $tbl_name(UserID,Pro_ID,Quy) values($UserID,$ProID,1)";
				$resultInsert=mysql_query($sqlInsert,$con);
				header('Location: index.php');
				}
		    else{
				$sqlUpdate="Update $tbl_name Set Quy=Quy+1 where WHERE UserID=$UserID and Pro_ID=$ProID";
